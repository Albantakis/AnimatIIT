...
if op_write > 0
    Foldername = strcat(TrialType,'_trial', int2str(TrialNum));
    mkdir(Foldername)
end 
...
for g = 16384 %0:16:30000-1
    ...
    %if ~isempty(TotNodes)
        ...
        SelfLoopElements = J_temp(J_temp(:,1)-J_temp(:,2)==0,1);
        ...
        if ~isempty(PosComplexes)
            for i = 1:length(PosComplexes)
                TempNodes = 1:numNodes;
                TryComplex = TempNodes(whichPosComp == PosComplexes(i));
                indexComplex(i) = subsystem2index(TryComplex);
            end
        end
        
        indexComplex = union(indexComplex, indexSelfLoop);
        indexComplex = sort(indexComplex);
...
%% new way of input with nodes
        ...
        in_nodes(2*num_nodes) = struct('num',2*num_nodes,'name',[num2str(used_nodes(num_nodes)-1) '_c'],'num_states',2,...
                                    'state_names',{{'0' '1'}},'logic_type',[],'cpt',[],...
...
%% initialize data which is the same for all states
        ...
        %             (network.full_system,this_state,tpm,network.b_table,network.options)
                ...
                elseif op_complex == 1
                    ...
                    [Big_phi_M phi_M prob_M M_cell concept_MIP_M purviews_M network] = big_phi_all(network, this_state);                                             
                    % complex search
